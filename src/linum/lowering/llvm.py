...
class SystemBackendLinker:
    ...
    @staticmethod
    def compile_to_object(llvm_ir: str, output_path: Optional[str] = None) -> bytes:
        cmd = ["llc", "-filetype=obj", "-o", "-"]
        proc = subprocess.run(cmd, input=llvm_ir.encode("utf-8"), capture_output=True)
        if proc.returncode != 0:
            raise RuntimeError(f"Object compilation failed: {proc.stderr.decode('utf-8')}")
        obj_bytes = proc.stdout
        if output_path:
