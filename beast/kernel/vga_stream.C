...
/* Spatial cursor tracking coordinate struct */
typedef struct {
    uint8_t x;
    uint8_t y;
} vga_cursor_t;
...
/**
 * Resets the display matrix to absolute void (clear screen).
 */
void vga_clear(void) {
...
/**
 * Scrolls the text matrix upward by a single row.
 * Preserves historical text geometry via block memory transposition.
 */
...
/**
 * Writes a single character to the explicit coordinate stream.
 * Handles carriage returns, line feeds, and automatic wrapping.
 */
void vga_putchar(char c) {
    if (c == '\n') {
        ...
        cursor.x = (cursor.x + 4) & ~3; /* Align to 4-column quipu grid */
...
/**
 * Renders a null-terminated prose stream into the structural terminal grid.
 */
