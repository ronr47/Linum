...
# 1. Audit SSA classes
inspect_file_ast(Path("src/linum/lowering/ssa.py"))

# 2. Audit CFG classes
inspect_file_ast(Path("src/linum/lowering/cfg.py"))

# 3. Print the exact SsaPhi definition from ssa.py
print(f"\n{'='*25} EXACT SsaPhi DEFINITION {'='*25}")
ssa_lines = Path("src/linum/lowering/ssa.py").read_text().splitlines()
in_phi = False
