...
class LlvmEmitter:
    ...
    def _get_type_str(self, var_name: str, var_types: dict, reg_types: dict = None) -> str:
        ...
        if any(k in clean.lower() for k in ["ptr", "buffer", "mem", "raw", "addr", "res_p", "stub", "alloc", "heap", "handle"]):
            return "ptr"
    ...
    def emit(self, cfg: Any, var_types: Dict[str, str]) -> str:
        ...
        for bb in blocks:
            for instr in bb.instructions:
                ...
                if cname in ("IrStore", "Store"):
                    ...
                    p_var = self.format_reg(getattr(instr, "var_name", None)).lstrip('%')
                    ptr_registers.add(p_var)
        ...
        def collect_operand(op: Optional[Any], force_ty: Optional[str] = None):
            ...
            if clean in defined_vars:
                return
            if ("." in clean or "_" in clean or clean.startswith("raw_") or clean.startswith("uninit_") or clean.startswith("int_") or clean.startswith("offset_") or clean.startswith("val_") or clean.startswith("cond_") or clean.startswith("size_") or clean.startswith("linear_")) and clean not in discovered_env_params:
