...
echo "=== 1. Checking CFG class names in src/linum/lowering/cfg.py ==="
$PY_BIN - <<'PY_EOF'
...
if cfg_file.exists():
    ...
    print("Found classes in cfg.py:", classes)
PY_EOF

echo ""
echo "=== 2. Writing corrected src/linum/lowering/llvm.py ==="
...
import linum.lowering.cfg as cfg_mod
    ...
    def emit(self, cfg: Any, var_types: Dict[str, str]) -> str:
        ...
        for bb in cfg.blocks:
            for instr in bb.instructions:
                if isinstance(instr, cfg_mod.IrAssign):
                    collect_operand(instr.src_reg)
                elif isinstance(instr, cfg_mod.IrLoad):
                    collect_operand(instr.src_var)
                elif isinstance(instr, cfg_mod.IrBinOp):
                    collect_operand(instr.left_reg)
                    collect_operand(instr.right_reg)
                elif isinstance(instr, cfg_mod.IrPtrOffset):
                    collect_operand(instr.base_ptr)
                    collect_operand(instr.offset_reg)
                elif isinstance(instr, cfg_mod.IrCall):
                    for arg in getattr(instr, "args_regs", ()):
                        collect_operand(arg)
            if isinstance(bb.terminator, cfg_mod.IrCondBranch):
                collect_operand(bb.terminator.cond_reg)
            elif isinstance(bb.terminator, cfg_mod.IrReturn):
                collect_operand(bb.terminator.val_reg)
        ...
        for bb in cfg.blocks:
            ...
            for instr in bb.instructions:
                ...
                elif isinstance(instr, cfg_mod.IrDrop):
                    self.lines.append(f"  call void @__drop_linear_resource(ptr %{instr.var_name.lstrip('%')})")

            if isinstance(bb.terminator, cfg_mod.IrBranch):
                self.lines.append(f"  br label %{bb.terminator.target_label}")
            elif isinstance(bb.terminator, cfg_mod.IrCondBranch):
                cond_formatted = self.resolve_operand(bb.terminator.cond_reg, discovered_env_params)
                ...
                if bb.terminator.val_reg:
                    ...
                    ret_fmt = self.resolve_operand(bb.terminator.val_reg, discovered_env_params)
