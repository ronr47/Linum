...
class LlvmEmitter:
    ...
    def _get_type_str(self, var_name: str, var_types: dict, reg_types: dict = None) -> str:
        ...
        # 1. Direct variable lookup (e.g., 'base_ptr', 'target')
        ...
        # 2. Versioned variable base lookup (e.g., 'base_ptr.1' -> 'base_ptr', 'val.3' -> 'val')
        base = clean.split('.')[0]
        ...
        # 4. Name-based heuristics for registers and parameters
        if "cond" in clean.lower() or "bool" in clean.lower():
            return "i1"

        if any(k in clean.lower() for k in ["ptr", "buffer", "mem", "raw", "addr", "res_p", "stub"]):
            return "ptr"
...
echo "Running pytest across all test suites..."
$PY_BIN -m pytest -vv
