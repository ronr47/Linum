...
# Overwrite matrix_dim_fail.linum with top-level block syntax
...
# Overwrite matrix_det_fail.linum with top-level block syntax
cat << 'INNER_EOF' > tests/programs/matrix_det_fail.linum
let A: matrix = matrix_new_non_square_placeholder;
let C: i64 = det(A);
INNER_EOF

echo "Test programs successfully updated to top-level syntax."
